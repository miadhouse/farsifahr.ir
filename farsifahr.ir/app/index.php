...
<head>
    ...
    <style>
        ...
        /* Make answer items flex column to stack content */
        .answer-item {
            display: flex;
...
<body style="height: 100%;background-color: #d3f5da;" class="<?= $mode === 'practice' ? 'practice-mode' : '' ?>">
    ...
    `;
            ...
            if (question.farsi_text && question.farsi_text.trim()) {
                questionTransContainer.innerHTML = `
                    <div class="translation-box">
                        <h6><i class="fas fa-language"></i> ترجمه سوال</h6>
        ...
        function showExplanationContent() {
            ...
            // بررسی اینکه آیا به سوال پاسخ داده شده است
            const hasAnswered = checkIfAnswered();
            
            // Show question explanation only if answered
            ...
            if (!hasAnswered) {
                questionExplainContainer.innerHTML = `
                    <div class="no-explanation">
                        <i class="fas fa-lock"></i> برای مشاهده توضیح، ابتدا به سوال پاسخ دهید
                    </div>
                `;
            } else if (question.info && question.info.trim()) {
                questionExplainContainer.innerHTML = `
            ...
            // Show pretext explanation if exists and answered
            const pretextExplainContainer = document.getElementById('pretext-explanation-container');
            if (!hasAnswered) {
                pretextExplainContainer.innerHTML = '';
            } else if (question.info && question.info.trim()) {
                pretextExplainContainer.innerHTML = `
                    <div class="pretext-explanation">
                        <strong><i class="fas fa-info-circle"></i> توضیح:</strong>
                        <p class="mb-0 mt-2">${question.info}</p>
                    </div>
                `;
            } else {
                pretextExplainContainer.innerHTML = '';
            ...
            // Show answer explanations only if answered
            if (hasAnswered && answers && answers.length > 0) {
                answers.forEach((answer, index) => {
                    const answerItem = document.querySelector(`[data-answer-index="${index}"]`);
        ...
        // تابع جدید برای بررسی اینکه آیا کاربر به سوال پاسخ داده است
        function checkIfAnswered() {
            // در حالت browse همیشه پاسخ داده شده محسوب می‌شود
            if (mode === 'browse') {
                return true;
            }
            
            // در حالت practice بررسی می‌کنیم
            if (mode === 'practice') {
                // اگر سوال حل شده باشد
                if (questionSolved) {
                    return true;
                }
                
                // بررسی نوع پاسخ
                if (currentQuestionData && currentQuestionData.answers && currentQuestionData.answers.length > 0) {
                    const answerType = currentQuestionData.answers[0]['asw_type'] || 1;
                    
                    if (answerType == 2) {
                        // پاسخ عددی - بررسی اینکه آیا چیزی وارد شده
                        const numericInput = document.getElementById('numeric-answer');
                        return numericInput && numericInput.value.trim().length > 0;
                    } else {
                        // پاسخ چندگزینه‌ای - بررسی اینکه آیا حداقل یک پاسخ انتخاب شده
        ...
        function solveQuestion() {
            ...
            // نمایش خودکار توضیح بعد از حل سوال
            if (!explanationActive) {
                // فعال کردن توضیح
                explanationActive = true;
                const explainBtn = document.getElementById('explainBtn');
                ...
                // غیرفعال کردن ترجمه اگر فعال بود
                if (translationActive) {
                    translationActive = false;
                    const translateBtn = document.getElementById('translateBtn');
                    translateBtn.classList.remove('active');
                ...
                // اگر توضیح قبلاً فعال بود، فقط محتوا را به‌روزرسانی کن
                showExplanationContent();
            }
        }
    ...
    `;
                    ...
                    answers.forEach((answer, index) => {
                        ...
                        answersText += `
            <div class="answer-item" data-answer-index="${index}">
        ...
        function handleAnswerChange(checkbox) {
            ...
            // اگر توضیح فعال است و کاربر پاسخ داد، توضیحات را به‌روزرسانی کن
            if (explanationActive) {
                showExplanationContent();
            }
        ...
        function clearLastChar() {
            ...
            if (mode === 'practice' && !questionSolved) {
                ...
                // اگر توضیح فعال است، توضیحات را به‌روزرسانی کن
                if (explanationActive) {
                    showExplanationContent();
                }
            }
        ...
        function clearAnswer() {
            ...
            if (mode === 'practice' && !questionSolved) {
                ...
                // اگر توضیح فعال است، توضیحات را به‌روزرسانی کن (پیام قفل نمایش داده می‌شود)
                if (explanationActive) {
                    showExplanationContent();
                }
            }
